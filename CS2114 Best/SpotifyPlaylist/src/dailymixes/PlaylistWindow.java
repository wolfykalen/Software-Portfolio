...
public class PlaylistWindow {
     ...
     * @param playlistCalculator a playlist calculator instance to pull queue 
     * and playlists from
     */
    public PlaylistWindow(PlaylistCalculator playlistCalculator) {
        window = new Window("Playlist Algorithm Generator");
        window.setSize((int) (500 * DISPLAY_FACTOR), (int) 
            (500 * DISPLAY_FACTOR));
    ...
    private void drawPlaylists() {
        ...
        drawPhone(PLAYLIST_X + PLAYLIST_SIZE + PLAYLIST_PADDING, 
            PLAYLIST_Y, 1);
    ...
    private void drawPhone(int x, int y, int playlistIndex) {
        ...
        for( int i = 0; i < Math.min(playlist.getNumberOfSongs(), 7); i++) {
            Shape song = new Shape(innerPhone.getX() + 5, songStartY + 
                (i * 25), innerPhone.getWidth() - 10, 20, 
    ...
    private Shape[] drawNote(int x, int y, Color color) {
        ...
        Shape circle = new CircleShape(x - MUSIC_NOTE_SIZE + bar.getWidth()
        + 1, y + bar.getHeight() - (MUSIC_NOTE_SIZE / 2), MUSIC_NOTE_SIZE, 
        color);
        note[0] = bar;
        note[1] = circle;
        window.addShape(bar);
    ...
    /**
     * This method updates the text objects that are associated with the 
     * queued song
     */
    ...
    private void updatePlaylistText() {
        for (int i = 0; i < playlistShapes.length; i++) {
            ...
            popText.setX(x + (playlistShape.getWidth() / 2) - 
                (popText.getWidth() / 2));
            ...
            rockText.setX(x + (playlistShape.getWidth() / 2) - 
                (rockText.getWidth() / 2));
            y += rockText.getHeight() + textMargin;
            message = "Country: " + playlist.getMinGenreSet().getCountry() 
